    ...
    if hands:
        ...
        try:
            ...
            if currentMode == MODE.Detection:
                ...
                if confidence >= 0.7:
                    cv2.putText(img, labels[index].split(' ')[1].replace('\n',''), (x + width, y - 25), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=COLOR_CONTRAST, thickness=2)
                else:
                    cv2.putText(img, '?', (x + width, y - 25), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=COLOR_CONTRAST, thickness=2)
